...
def process_dynamic_items_table(template_id, document_name, substitutions):
    ...
    # Making a copy of the Google Docs template to create the new albaran
    ...
    if table_start_index == -1 or start_row_index == -1:
        raise ValueError(f'❌ Não foi possível encontrar a tabela ou o marcador {START_TAG}.')
    ...
    # === 2️⃣ Verifica quantos itens existem ===
    item_indices = []
    for key in substitutions.keys():
        if key.startswith("description") and key[-1].isdigit():
            try:
                idx = int(key.replace("description", ""))
...
ALBA_TEMPLATE_ID = st.secrets.google_definition["ALBA_TEMPLATE_ID"]
ALBA_PDF_FOLDER_ID = st.secrets.google_definition["ALBA_PDF_FOLDER_ID"]
# for Google SHEETS
...
df_albarans = leitura_worksheet('albarans')

# Add client name (nombre_cliente) column mapping by client unique code (cod_cliente)
df_albarans['nombre_cliente'] = df_albarans.cod_cliente.map(
    df_clientes_activos.set_index('cod_cliente')['nombre_cliente'].to_dict())

# Adjusting the float numbers to match european standard
df_albarans['cantidad'] = df_albarans['cantidad'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))
df_albarans['precio_unit'] = df_albarans['precio_unit'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))
df_albarans['base_imponible'] = df_albarans['base_imponible'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))
df_albarans['cuota_tributaria'] = df_albarans['cuota_tributaria'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))
df_albarans['valor_retencion'] = df_albarans['valor_retencion'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))
df_albarans['total'] = df_albarans['total'].apply(
    lambda x: float(str(x).replace('.', '').replace(',', '.')))


# Group all records with the same invoice number and get the sum
df_total_albarans = df_albarans.groupby(['nro_alba'], as_index=False).agg(
    {'total': 'sum', 'nombre_cliente': 'first', 'descripcion': 'first', 'fecha_emision': 'first'})
df_total_albarans['total'] = df_total_albarans['total'].apply(lambda x: f'{x:.2f}')
df_total_albarans.index += 1  # make index start at 1

# Tabs
tab_options = ['Crear nuevo', 'Listar todos']
TAB_1 = tab_options[0]
TAB_2 = tab_options[1]
...
if tab == TAB_1:
    ...
    # last_albaran_row = len(df_albarans)  # get the last written row from the dataframe
    # last_albaran = df_albarans.loc[last_albaran_row, 'nro_alba']  # get Budget number from column 'nro_alba'
    # albaran_nr = str(int(last_albaran) + 1).zfill(4)  # zfill=4 format 9999
    if not df_albarans.empty:
        last_albaran = int(df_albarans['nro_alba'].astype(int).max())
    else:
        last_albaran = 0
    albaran_nr = str(last_albaran + 1).zfill(4)  # zfill=4 format 9999
    ...
    st.subheader('Nuevo Albaran: ' + albaran_nr)
    ...
    if client is not None:
        ...
        if 'df_to_save' in state:
            if st.button('Preencher linhas com dados importados', type='primary'):
                items_df = state.df_to_save
                num_rows = len(state.df_to_save)
                state.num_rows_input = num_rows
                for i, row in items_df.iterrows():
                    state[f'albaran_description{i}'] = row['Descripción']
                    state[f'albaran_qty{i}'] = float(row['Cantidad'])
                    state[f'albaran_value{i}'] = float(row['Valor'])
        ...
        # with st.form('albaran_form'):
            ...
            with col2:
                form_albaran_iva = st.number_input('% IVA', min_value=0, max_value=21, step=21, value=21)
            with col3:
                form_albaran_desconto = st.number_input('% Retención', min_value=0, value=0)
            ...
            for row in range(num_rows):
                ...
                with grid[3]:
                    ...
                    base_key = f'albaran_base{row}'
                    state[base_key] = albaran_base_imponible
                    line_base = st.number_input('Base imp.', format="%0.2f", disabled=True, key=base_key)
                with grid[4]:
                    albaran_cuota_tributaria = albaran_base_imponible * form_albaran_iva / 100
                    # cuota_key = f'albaran_cuota{row}_{line_qty}_{line_value}_{form_albaran_iva}'
                    cuota_key = f'albaran_cuota{row}'
                    state[cuota_key] = albaran_cuota_tributaria
                    line_cuotatrib = st.number_input('Cuota trib.', format="%0.2f",
                                                     disabled=True, key=cuota_key)
                with grid[5]:
                    albaran_valor_retencion = albaran_base_imponible * form_albaran_desconto / 100
                    # retencion_key = f'albaran_retencion{row}_{line_qty}_{line_value}_{valor_retencion}'
                    retencion_key = f'albaran_retencion{row}'
                    state[retencion_key] = albaran_valor_retencion
                    line_retencion = st.number_input('Val.ret.', format="%0.2f",
                                                     disabled=True, key=retencion_key)
                with grid[6]:
                    albaran_total = albaran_base_imponible + albaran_cuota_tributaria - albaran_valor_retencion
                    # total_key = f'albaran_total{row}_{line_qty}_{line_value}_{total}'
                    total_key = f'albaran_total{row}'
                    state[total_key] = albaran_total
                    line_total = st.number_input('Total', format="%0.2f", disabled=True,
                                                 key=total_key)
                    total_albaran += albaran_total
                    base_imponible_sum += albaran_base_imponible
                    cuota_tributaria_sum += albaran_cuota_tributaria
                    valor_retencion_sum += albaran_valor_retencion

            # field to enter the albaran Note
            if form_albaran_iva == 0:  # if IVA=0% then this note is obligatory in Spain
                albaran_nota_iva0 = 'Operación de inversión del sujeto pasivo de acuerdo al artículo 84, apartado uno,' \
                                   ' número 2o.f de la Ley 37/92 de IVA.'
            else:
                albaran_nota_iva0 = ''

            form_albaran_note = st.text_area('Nota:', value=albaran_nota_iva0,
                                            placeholder='Introduzca una nota para incluir en el presupuesto...')

            # area to display the total amount of the albaran
            ...
            with col2:
                ...
                total_albaran_formatado = "€ {:,.2f}".format(total_albaran).replace(",", "X").replace(".",
                                                                                                    ",").replace(
                    "X", ".")
                st.metric(label='TOTAL', value=total_albaran_formatado)
            ...
            if total_albaran != 0:
                ...
                for i in range(num_rows):
                    ...
                    description = state[f'albaran_description{i}']
                    qty = state[f'albaran_qty{i}']
                    value = state[f'albaran_value{i}']
                    base = state[f'albaran_base{i}']
                    cuota = state[f'albaran_cuota{i}']
                    ...
                    row = [
                        ...
                        base,                           # base imponible
                        form_albaran_iva / 100,         # percentual IVA
                        cuota,                          # cuota tributaria
                        form_albaran_desconto / 100,    # percentual retención
                        retencion,                      # valor retención
                        total,                          # total del presupuesto
                        form_albaran_note               # nota
                    ]
                    registro.append(row)
                ...
                substituicoes = {
                    'alba_nr': str(albaran_nr),
                    'alba_date': albaran_date,
                    'client': client,
                    ...
                    'total_alba': str(total_albaran_formatado)
                }

                # Dynamically add the items (lines) of the invoice service descriptions in the dictionary
                for idx in range(num_rows):
                    description = state[f'albaran_description{idx}']
                    qty = state[f'albaran_qty{idx}']
                    value = state[f'albaran_value{idx}']
                    value_formatado = "{:,.2f}".format(value).replace(",", "X").replace(".", ",").replace(
                        "X", ".")
                    base = state[f'albaran_base{idx}']
                ...
                # New document name
                new_document_name = f'Albaran-{albaran_nr}'
                ...
                with action2:  # SAVE albaran button
                    add_albaran = st.button('Salvar Albaran', type='primary', use_container_width=True)

                if add_albaran: # save the albaran (presupuesto)
                    # Replace the placeholders in the document
                    new_document_id, _ = process_dynamic_items_table(
                        template_id=ALBA_TEMPLATE_ID,
                        document_name=new_document_name,
                    ...
                    pdf_metadata = {
                        ...
                        'parents': [ALBA_PDF_FOLDER_ID] # coloca o arquivo PDF dentro da pasta específica
                    }
                    media = googleapiclient.http.MediaIoBaseUpload(io.BytesIO(pdf_request.execute()),
                                                                   mimetype='application/pdf')
                    # print("salvando o PDF na pasta 'Albarans' no Google Drive...")
                    ...
                    st.success(f'Albaran {albaran_nr} creado con éxito.', icon='✅')
                    # print(f" state antes:\n {state}")
...
if tab == TAB_2:
    st.divider()
    # Show ALL albarans
    st.subheader('Todos Albarans')
    st.dataframe(df_total_albarans)
